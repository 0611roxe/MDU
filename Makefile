...
verilog: $(SCALA_FILE)
	...
	mill -i $(TOP).runMain $(MAIN) -td $(BUILD_DIR)/systemverilog --emit-modules systemverilog
...
sim: $(CSRCS) verilog
	@rm -rf $(OBJ_DIR)
	$(VERILATOR) $(VERILATOR_FLAGS) -top $(TOPNAME) $(CSRCS) $(wildcard $(BUILD_DIR)/systemverilog/*.sv) \
